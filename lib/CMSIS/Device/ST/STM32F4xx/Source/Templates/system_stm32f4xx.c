...
#    define HSE_VALUE \
        ((uint32_t)25000000) /*!< Default value of the External oscillator in \
                                Hz */
#endif /* HSE_VALUE */
...
void SystemClock_Config(void)
{
    ...
    /* Set voltage scale to maximum performance */
    PWR->CR |= PWR_CR_VOS;

    /* Set Flash wait states for 96MHz (3 wait states at 3.3V) */
    FLASH->ACR = FLASH_ACR_ICEN | FLASH_ACR_DCEN | FLASH_ACR_PRFTEN |
                 FLASH_ACR_LATENCY_3WS;

    /* Configure PLL: 96MHz SYSCLK, 48MHz USB */
